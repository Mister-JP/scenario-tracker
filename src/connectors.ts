...
/** A connection "dot" on the edge of a card */
interface EndPt {
  card: HTMLElement;       // the parent card element
  el: HTMLDivElement;      // the dot itself (for geometry)
}

/** One arrow (SVG <line>) linking two EndPts */
interface Line {
...
const endpoints: EndPt[] = [];   // every dot on every card
const lines: Line[] = [];        // every arrow currently on screen
...
export function registerEndPoints(
  card: HTMLElement,
  dots: { el: HTMLDivElement }[]
) {
  dots.forEach(d => {
    const ep: EndPt = { card, el: d.el };
    endpoints.push(ep);
...
let drawing: { from: EndPt; line: SVGLineElement } | null = null;
const SNAP = 24;               // px radius inside which we auto-attach

function startDraw(from: EndPt, x: number, y: number) {
...
function nearestEndpoint(x: number, y: number, skip: EndPt) {
  let best: EndPt | null = null;
  let bestD = SNAP;
  endpoints.forEach(ep => {
    if (ep.card === skip.card) return;      // can't connect to self
    const c = center(ep);
...
function finishDraw(from: EndPt, to: EndPt, line: SVGLineElement) {
...
function markDotAsOccupied(dot: HTMLDivElement) {
...
function isDotOccupied(dot: HTMLDivElement): boolean {
  return dot.dataset.occupied === "true";
}

/* Create dots adjacent to occupied dots */
function addAdjacentDots(card: HTMLElement, occupiedDot: HTMLDivElement) {
...
function createIntermediateDots(card: HTMLElement, side: number) {
  // Get all dots on this side
  const dotsOnSide = Array.from(card.querySelectorAll(`.dot[data-side="${side}"]`)) as HTMLDivElement[];
...
function createEdgeIntermediateDots(card: HTMLElement, side: number, existingDots: HTMLDivElement[]) {
  // Sort dots by position
  let sortedDots = sortDotsByPosition(side, existingDots);
  
  // Find gaps where we could add dots
  let newDotPositions: {left: string, top: string}[] = [];
...
function sortDotsByPosition(side: number, dots: HTMLDivElement[]): HTMLDivElement[] {
...
function getDotPosition(dot: HTMLDivElement): number {
...
function createNewDot(card: HTMLElement, side: number, left: string, top: string) {
  // Check if a dot already exists very close to this position
  const existingDots = Array.from(card.querySelectorAll('.dot')) as HTMLDivElement[];
...
function getPercent(value: string): number {
  const match = value.match(/^([\d.]+)%$/);
  return match ? parseFloat(match[1]) / 100 : 0;
}

/* Export function to remove a line by ID (used by dialog) */
export function removeLineById(id: string) {
...
function getSideFromDot(dot: HTMLDivElement): number {
  ...
  if (top === "0%") return 0;       // top
  if (left === "100%") return 1;     // right
  if (top === "100%") return 2;      // bottom
  if (left === "0%") return 3;       // left
  
  // Corners
  if (left === "0%" && top === "0%") return 4;       // top-left
  if (left === "100%" && top === "0%") return 5;     // top-right
  if (left === "100%" && top === "100%") return 6;   // bottom-right
  if (left === "0%" && top === "100%") return 7;     // bottom-left
  
  return 0; // Default
}

/* Create a line from saved data */
export function createLineFromSaved(
  fromCardId: number, 
  fromSide: number,
  toCardId: number,
  toSide: number
  ...
  // Find the cards
  const fromCard = document.querySelector(`.card[data-id="${fromCardId}"]`) as HTMLElement;
  const toCard = document.querySelector(`.card[data-id="${toCardId}"]`) as HTMLElement;
...
function findDotAtSide(card: HTMLElement, side: number): HTMLDivElement | null {
  ...
  return dots[0] as HTMLDivElement;
}
