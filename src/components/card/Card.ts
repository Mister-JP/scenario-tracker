 ...
 */
export function createCard(scenarioId: number, position: Position): void {
  // Create the card container
  const card = createElement<HTMLDivElement>("div", "card", {
  ...
  // Create drag handle
  const handle = createElement<HTMLDivElement>("div", "handle");
 ...
 */
function createConnectorDots(card: HTMLElement): ConnectionEndpoint[] {
  // Define dot positions [left%, top%, sideIndex]
  const dotDefinitions: [string, string, number][] = [
    // Edge midpoints
    ["50%", "0%", DotSide.TOP],
  ...
  dotDefinitions.forEach(([left, top, side]) => {
    const dot = createElement<HTMLDivElement>("div", "dot", {
 ...
 */
function setupIframeScaling(card: HTMLElement, iframe: HTMLIFrameElement): void {
  const rescale = (): void => {
 ...
 */
function setupDragBehavior(card: HTMLElement, handle: HTMLElement): void {
