...
import { recalcAllLines, // recalculates arrow positions on every move/resize
...
/* ------------------------------------------------------------------ */
export function createCard(n, pos) {
    /* ---------- outer shell ---------- */
    ...
    /* ---------- drag behaviour ---------- */
    let drag = false;
    let off = { x: 0, y: 0 }; // pointer offset from card's top-left
    ...
    handle.addEventListener("pointermove", e => {
        ...
        if (y < headerOffset())
            y = headerOffset(); // clamp under header
