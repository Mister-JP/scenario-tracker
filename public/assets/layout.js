...
import { recalcAllLines, getLines, clearAllLines, createLineFromSaved } from "./connectors.js";
...
export function applyLayout(json) {
    // Parse the JSON layout
    const layout = JSON.parse(json);
    const top = headerOffset();
    // Apply card positions and sizes
    if (layout.cards) {
        layout.cards.forEach((cfg) => {
            const el = store.cards.get(cfg.n);
            if (!el)
                return; // unknown scenario number
    ...
    if (layout.arrows) {
        layout.arrows.forEach((arrow) => {
            createLineFromSaved(arrow.fromCard, arrow.fromSide, arrow.toCard, arrow.toSide);
        });
    }
    recalcAllLines();
}
