...
import { getAllConnections, clearAllConnections, createConnectionFromSaved, recalculateAllLines } from "../../components/connector/ConnectorManager";
/**
...
export function createLayoutSnapshot() {
    ...
    const cardSpecs = Array.from(store.cards.entries()).map(([scenarioId, element]) => {
 ...
 */
export function saveLayoutToFile(fileName) {
    // Create JSON blob
 ...
 */
export function loadLayoutFromFile(json) {
    try {
        // Parse the JSON
        ...
        if (layout.arrows) {
            layout.arrows.forEach((connection) => {
                createConnectionFromSaved(connection.fromCardId, connection.fromSide, connection.toCardId, connection.toSide);
            });
        }
        // Update connector lines
        recalculateAllLines();
